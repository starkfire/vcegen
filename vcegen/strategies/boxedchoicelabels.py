...
class BoxedChoiceLabelsStrategy:

    def __init__(self, input_file, blacklist: list[str] = [], debug=False):
    ...
    def __parse_row(self, row: list, blacklist: list[str] = []):
        entry = {
    ...
    def __run_strategy(self, pdf: pdfplumber.pdf.PDF):
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                for row in table:
                    output = self.__parse_row(row, blacklist=self.blacklist)
    ...
    def validate(self, min_choices=3, auto_filter=True):
        invalid = []
        original_count = len(self.result)
